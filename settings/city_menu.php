...
$sql_code = "SELECT * FROM cidades WHERE ativo = 1";
            ...
            <tbody>
                <?php while ($row = $sql_query->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <a href="city_edit.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary">Editar</a>
                            <a href="city_delete.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Tem certeza que deseja excluir?');">Desativar</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
