...
$stmt = $mysqli->prepare("SELECT * FROM setores WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome)) {
        ...
        $stmt_up = $mysqli->prepare("UPDATE setores SET nome = ? WHERE id = ?");
        ...
        if ($stmt_up->execute()) {
            ...
            $erro = 'Erro ao atualizar setor: ' . $mysqli->error;
        }
    }
}
?>
        ...
        <div class="content">
            <div class="page-header">
                <div>
                    ...
                    <h1 class="page-title"><strong>Editar</strong> Setor</h1>
                </div>
                <a href="sector_menu.php" class="btn btn-secondary">
            ...
            <div class="card" style="max-width:480px;margin:0 auto;">
                <div class="card-header">
                    <span class="card-header-title"><i class="fas fa-sitemap"></i> Dados do Setor</span>
                </div>
                <div class="card-body">
                    <form method="POST" action="">
                        <div class="form-group">
                            <label class="form-label">Nome do Setor <span class="form-required">*</span></label>
                            <input type="text" class="form-control" name="nome" required
                                   value="<?= htmlspecialchars($setor['nome'], ENT_QUOTES, 'UTF-8') ?>">
                        </div>
